 ...
 */
 ...
 int main(int argc, char* argv[]) {
   ...
   switch (FLAGS_dataset_type) {
     case 0: {
       switch (vio_params.frontend_type_) {
         ...
         case VIO::FrontendType::kGnssStereoImu: {
           dataset_parser = std::make_unique<VIO::GNSSVIODataProvider>(vio_params);
         } break;
         default: {
   ...
   switch (vio_params.frontend_type_) {
     ...
     case VIO::FrontendType::kGnssStereoImu: {
       vio_pipeline = std::make_unique<VIO::GnssStereoImuPipeline>(vio_params);
     } break;
     default: {
       LOG(FATAL) << "Unrecognized Frontend type: "
                  << VIO::to_underlying(vio_params.frontend_type_)
   ...
   if (vio_params.frontend_type_ == VIO::FrontendType::kGnssStereoImu) {
    auto stereo_pipeline =
        std::dynamic_pointer_cast<VIO::GnssStereoImuPipeline>(vio_pipeline);
    CHECK(stereo_pipeline);
    ...
    dataset_parser->registerRightFrameCallback(
        std::bind(&VIO::GnssStereoImuPipeline::fillRightFrameQueue,
                  stereo_pipeline,
                  std::placeholders::_1));
    dataset_parser->registerGnssCallback(
...
//   // Parse VIO parameters from gflags.
//   VIO::VioParams vio_params(FLAGS_params_folder_path);
//   LOG(INFO) << "THIS IS FRONTEND: " << VIO::to_underlying(vio_params.frontend_type_);
//   // Build dataset parser.
//   VIO::DataProviderInterface::Ptr dataset_parser = nullptr;
//   switch (FLAGS_dataset_type) {
//     case 0: {
...
//     VIO::GnssStereoImuPipeline::Ptr stereo_pipeline = 
//       std::dynamic_pointer_cast<VIO::GnssStereoImuPipeline>(vio_pipeline);
