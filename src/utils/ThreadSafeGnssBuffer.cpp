...
ThreadsafeGnssBuffer::getInterpolatedValueWithFixedPeriod(
  ...
  size_t index =
      static_cast<size_t>((timestamp_ns - oldest.timestamp_) / gnss_period_ns);
  ...
  if (!buffer_.getValueAtOrBeforeTime(
          ts_lower, &pre_border_timestamp, &pre_border_value)) {
    LOG(WARNING)
  ...
  if (!buffer_.getValueAtOrAfterTime(
          ts_upper, &post_border_timestamp, &post_border_value)) {
    LOG(WARNING)
...
ThreadsafeGnssBuffer::QueryResult
ThreadsafeGnssBuffer::getGnssDataBtwTimestamps(
    const Timestamp& timestamp_ns_from,
  ...
  CHECK(buffer_.getValuesBetweenTimes(
      timestamp_ns_from, timestamp_ns_to, &between_values, get_lower_bound));
...
ThreadsafeGnssBuffer::getGnssDataInterpolatedUpperBorder(
  ...
  // Get data.
  QueryResult query_result =
      getGnssDataBtwTimestamps(timestamp_ns_from,
                               timestamp_ns_to,
                               gnss_timestamps,
                               gnss_points,
...
ThreadsafeGnssBuffer::QueryResult ThreadsafeGnssBuffer::getNearestGnssData(
    const Timestamp& timestamp_ns_to,
    GnssStampS* gnss_timestamps,
    GnssPointS* gnss_points) {
  Timestamp gnss_timestamp;
  GnssMeasurement gnss_point;
  ThreadsafeGnssBuffer::QueryResult query_result;
  if (buffer_.getValueAtOrBeforeTime(
          timestamp_ns_to, &gnss_timestamp, &gnss_point)) {
    gnss_timestamps->resize(Eigen::NoChange, 1);  // 1 timestamp (1x1)
    gnss_points->resize(Eigen::NoChange, 1);      // 6x1 point

    (*gnss_timestamps)(0, 0) = gnss_timestamp;
    gnss_points->col(0) = gnss_point.point_;
    query_result = ThreadsafeGnssBuffer::QueryResult::kDataAvailable;
  } else {
    query_result = ThreadsafeGnssBuffer::QueryResult::kDataNotYetAvailable;
  }
  return query_result;
}
...
ThreadsafeGnssBuffer::getGnssDataInterpolatedBorders(
  ...
  QueryResult query_result = getGnssDataBtwTimestamps(timestamp_ns_from,
                                                      timestamp_ns_to,
                                                      &gnss_timestamps_tmp,
                                                      &gnss_points_tmp,
                                                      true);
  ...
  // Add points.
  gnss_timestamps->middleCols(1, gnss_timestamps_tmp.cols()) =
      gnss_timestamps_tmp;
  gnss_points->middleCols(1, gnss_points_tmp.cols()) = gnss_points_tmp;
  // Append upper border.
  gnss_timestamps->rightCols<1>()(0) = timestamp_ns_to;
  gnss_points->rightCols<1>() = interpolated_upper_border;
...
void ThreadsafeGnssBuffer::interpolateValueAtTimestamp(
  ...
  Timestamp pre_border_timestamp, post_border_timestamp;
  GnssMeasurement pre_border_value, post_border_value;
  CHECK(buffer_.getValueAtOrBeforeTime(
      timestamp_ns, &pre_border_timestamp, &pre_border_value))
      << "The GNSS buffer seems not to contain points at or before time: "
      << timestamp_ns;
  CHECK_EQ(pre_border_timestamp, pre_border_value.timestamp_);
  CHECK(buffer_.getValueAtOrAfterTime(
...
ThreadsafeGnssBuffer::getGnssDataInterpolatedBordersBlocking(
  ...
  return getGnssDataInterpolatedBorders(
