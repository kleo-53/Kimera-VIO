 ...
 */
 ...
 DECLARE_bool(do_coarse_gnss_camera_temporal_sync);
 DECLARE_bool(do_fine_gnss_camera_temporal_sync);
 
 namespace VIO {
 
 GnssStereoImuPipeline::GnssStereoImuPipeline(const VioParams& params,
   ...
   //! Create DataProvider
   data_provider_module_ = std::make_unique<GnssStereoDataProviderModule>(
   ...
   if (FLAGS_do_coarse_gnss_camera_temporal_sync) {
    data_provider_module_->doCoarseGnssCameraTemporalSync();
  }
  if (!FLAGS_do_fine_gnss_camera_temporal_sync) {
    if (FLAGS_do_coarse_gnss_camera_temporal_sync) {
      LOG(WARNING) << "The manually provided GNSS time shift will be applied on "
                      "top of whatever the coarse time alignment calculates. "
                      "This may or may not be what you want!";
    }
    data_provider_module_->setGnssTimeShift(params.gnss_params_.timestamp_);
  }
   ...
   //! Create Frontend
   vio_frontend_module_ = std::make_unique<VisionImuFrontendModule>( // TOD: СЮДА GNSS добавить
       &frontend_input_queue_,
       parallel_run_,
   ...
   vio_frontend_module_->registerImuTimeShiftUpdateCallback(
    ...
    vio_frontend_module_->registerGnssTimeShiftUpdateCallback(
        [&](double gnss_time_shift_s) {
            data_provider_module_->setGnssTimeShift(gnss_time_shift_s);
        });
     ...
     display_module_ = std::make_unique<DisplayModule>(
         ...
         displayer ? std::move(displayer)
                   : DisplayFactory::makeDisplay(
                         ...
                         std::bind(&GnssStereoImuPipeline::shutdown, this)));
   }
...
//   //! Create Stereo Camera
//   CHECK_EQ(params.camera_params_.size(), 2u)
//       << "Need two cameras for GnssStereoImuPipeline.";
//   stereo_camera_ = std::make_shared<StereoCamera>(params.camera_params_.at(0),
//                                                   params.camera_params_.at(1));
...
// //   if (params.gnss_params_) {
//     data_provider_module_->setGnssTimeShift(
//         params.gnss_params_.timestamp_);
// //   }
//   if (FLAGS_do_coarse_gnss_camera_temporal_sync) {
//     data_provider_module_->doCoarseGnssCameraTemporalSync();
//   }

//   LOG(WARNING) << "FLAG TO VIS" << FLAGS_visualize;
...
//                 CHECK(converted_output);
//                 // LOG(INFO) << "Backend output received. TS: " << output->timestamp_;
...
//                         converted_output->stereo_frame_lkf_.timestamp_,
//                         converted_output->status_stereo_measurements_,
//                         //   converted_output.getTrackerStatus(),
//                         converted_output->pim_,
//                         converted_output->imu_acc_gyrs_,
//                         converted_output->body_lkf_OdomPose_body_kf_,
//                         converted_output->body_kf_world_OdomVel_body_kf_));
...
//             std::dynamic_pointer_cast<StereoFrontendOutput>(output);
//             CHECK(converted_output);
//             LOG(INFO) << "OUTPUT: " << converted_output;
//             CHECK_NOTNULL(visualizer_module.get())
//             ->fillFrontendQueue(converted_output);
//         });
...
//   LOG(INFO) << "GnssStereoImuPipeline initialized.";
// //   LOG(INFO) << "Visualizer module: " << (visualizer_module_ ? "OK" : "NULL");
// //   LOG(INFO) << "Backend module: " << (vio_backend_module_ ? "OK" : "NULL");
// //   LOG(INFO) << "Mesher module: " << (mesher_module_ ? "OK" : "NULL");
// // LOG(INFO) << "Backend input queue size: " << backend_input_queue_;
// // LOG(INFO) << "Display input queue size: " << display_input_queue_;
// }
