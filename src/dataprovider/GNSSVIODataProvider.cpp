...
DECLARE_int64(initial_k);
DECLARE_int64(final_k);
DECLARE_bool(log_euroc_gt_data);
...
GNSSVIODataProvider::GNSSVIODataProvider(const VioParams& vio_params)
    : GNSSVIODataProvider(FLAGS_dataset_path,
                          FLAGS_initial_k,
                          FLAGS_final_k,
                          vio_params) {}
...
bool GNSSVIODataProvider::spin() {
  ...
  LOG_IF(INFO, shutdown_) << "GNSS DataProvider shutdown requested.";
  return false;
}

bool GNSSVIODataProvider::hasData() const { return current_k_ < final_k_; }
...
void GNSSVIODataProvider::sendImuData() const {
...
/* -------------------------------------------------------------------------- */
void GNSSVIODataProvider::parse() {
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::parseImuData(const std::string& input_dataset_path,
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::parseGtData(const std::string& input_dataset_path,
...
bool GNSSVIODataProvider::parseGnssData(const std::string& input_dataset_path,
  ...
  while (std::getline(fin, line)) {
    ...
    if (!vio_params_.gnss_params_.b_pose_gnss_.equals(gtsam::Pose3())) {
      gtsam::Vector4 p_sensor_hom(
          gnss_data_raw[0], gnss_data_raw[1], gnss_data_raw[2], 1);
      gtsam::Matrix4 T_BS_mat =
          vio_params_.gnss_params_.b_pose_gnss_.inverse().matrix();
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::parseDataset() {
  // Parse IMU data.
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::parseCameraData(const std::string& cam_name,
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::sanityCheckCameraData(
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::sanityCheckCamSize(
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::sanityCheckCamTimestamps(
...
std::string GNSSVIODataProvider::getDatasetName() {
...
/* -------------------------------------------------------------------------- */
gtsam::Pose3 GNSSVIODataProvider::getGroundTruthRelativePose(
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::isGroundTruthAvailable(
    const Timestamp& timestamp) const {
  return timestamp > gt_data_.map_to_gt_.begin()->first;
}

/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::isGroundTruthAvailable() const {
  return is_gt_available_;
}

/* -------------------------------------------------------------------------- */
VioNavState GNSSVIODataProvider::getGroundTruthState(
    const Timestamp& timestamp) const {
  const auto& it_low =
...
GNSSVIODataProvider::getInitializationPerformance(
...
/* -------------------------------------------------------------------------- */
size_t GNSSVIODataProvider::getNumImages() const {
...
/* -------------------------------------------------------------------------- */
size_t GNSSVIODataProvider::getNumImagesForCamera(
...
/* -------------------------------------------------------------------------- */
bool GNSSVIODataProvider::getImgName(const std::string& camera_name,
                                     const size_t& k,
...
/* -------------------------------------------------------------------------- */
Timestamp GNSSVIODataProvider::timestampAtFrame(const FrameId& frame_number) {
...
void GNSSVIODataProvider::clipFinalFrame() {
...
/* -------------------------------------------------------------------------- */
void GNSSVIODataProvider::print() const {
