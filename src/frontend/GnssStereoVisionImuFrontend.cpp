 ...
 */
 ...
 StereoFrontendOutput::UniquePtr GnssStereoVisionImuFrontend::bootstrapSpinStereo(
   ...
   frontend_state_ = FLAGS_do_fine_imu_camera_temporal_sync
    ...
    LOG(INFO) << "STATE CHANGE TO NOMINAL(?)";
 ...
 void GnssStereoVisionImuFrontend::processFirstStereoFrame(
 ...
 // misleading
 StatusStereoMeasurementsPtr GnssStereoVisionImuFrontend::processStereoFrame(
 ...
 void GnssStereoVisionImuFrontend::getSmartStereoMeasurements(
 ...
 /* -------------------------------------------------------------------------- */
 void GnssStereoVisionImuFrontend::sendFeatureTracksToLogger() const {
   CHECK(tracker_);
 ...
 /* -------------------------------------------------------------------------- */
 void GnssStereoVisionImuFrontend::sendStereoMatchesToLogger() const {
   CHECK(tracker_);
 ...
 /* -------------------------------------------------------------------------- */
 void GnssStereoVisionImuFrontend::sendMonoTrackingToLogger() const {
 ...
 // RANSAC
 gtsam::Pose3 GnssStereoVisionImuFrontend::getRelativePoseBodyMono() const {
 ...
 // RANSAC
 gtsam::Pose3 GnssStereoVisionImuFrontend::getRelativePoseBodyStereo() const {
   gtsam::Pose3 body_Pose_cam_ =
       stereo_camera_->getBodyPoseLeftCamRect();  // of the left camera!!
 ...
 void GnssStereoVisionImuFrontend::printStatusStereoMeasurements(
...
//   CHECK(input);
//   GnssStereoFrontendInputPayload* raw_gnss_input =
//       dynamic_cast<GnssStereoFrontendInputPayload*>(input.get());
//   CHECK(raw_gnss_input) << "Invalid input payload type";
//   GnssStereoFrontendInputPayload::UniquePtr gnss_vio_input(raw_gnss_input);
//   input.release();  // Передаем владение
...
//   // Второе приведение: FrontendOutputPacketBase → StereoFrontendOutput
//   StereoFrontendOutput* raw_vio_output =
//       dynamic_cast<StereoFrontendOutput*>(vio_output_base.get());
//   CHECK(raw_vio_output) << "Invalid output type";
//   StereoFrontendOutput::UniquePtr vio_output(raw_vio_output);
//   vio_output_base.release();  // Передаем владение
  
//   return std::make_unique<GnssStereoFrontendOutput>(std::move(vio_output),
//                                                    gnss_nav_data);
