...
/**
 * @file   testGnssStereoProvider.cpp
 * @brief  test the GnssStereoDataProvider, especially around timestamp
 * invariants
...
class TestGnssStereoProvider : public ::testing::Test {
 public:
  TestGnssStereoProvider() : last_id_(0), output_queue_("test_output") {
    test_provider_ = std::make_unique<GnssStereoDataProviderModule>(
        &output_queue_,
        "test_gnss_stereo_provider",
        false,
  ...
  ~TestGnssStereoProvider() = default;

 protected:
  DataProviderModule::OutputQueue output_queue_;
  GnssStereoDataProviderModule::UniquePtr test_provider_;
  FrameId last_id_;
  ...
  void addGnss(Timestamp timestamp) {
    test_provider_->fillGnssQueue(
        GnssMeasurement(timestamp, GnssPoint(1, 2, 3)));
  }
...
TEST_F(TestGnssStereoProvider, basicSequentialCase) {
  ...
  auto result = castUnique<GnssStereoImuSyncPacket>(std::move(result_base));

  GnssPoint expected_gnss_point(GnssPoint(1, 2, 3));
  ASSERT_TRUE(result != nullptr);
  EXPECT_EQ(expected_gnss_point, result->getGnssPoint());

  ASSERT_TRUE(result != nullptr);
  EXPECT_EQ(static_cast<FrameId>(1), result->getStereoFrame().id_);
}

TEST_F(TestGnssStereoProvider, dropFramesOlderThanImu) {
  addImu(10);
  addGnss(10);
  addFrame(11);
  spinWithTimeout();
  ...
  auto result_g = castUnique<GnssStereoImuSyncPacket>(std::move(result));
  ASSERT_TRUE(result_g != nullptr);
  EXPECT_EQ(GnssPoint(1, 2, 3), result_g->getGnssPoint());

  ImuStampS expected_imu_times(1, 2);
  expected_imu_times << 11, 16, 17;
  EXPECT_EQ(expected_imu_times, result_g->imu_stamps_);
}

TEST_F(TestGnssStereoProvider, manyImuTest) {
  Timestamp t_curr = 10;
  Timestamp num_imu = 5;
...
TEST_F(TestGnssStereoProvider, imageBeforeImuTest) {
...
TEST_F(TestGnssStereoProvider, imageBeforeImuAndGnssTest) {
  // Drop any frame that appears before the IMU packets do
  addFrame(10);
  spinWithTimeout();
  EXPECT_TRUE(output_queue_.empty());
...
TEST_F(TestGnssStereoProvider, imageBeforeImuDelayedSpinTest) {
  // Drop any frame that appears before the IMU packets do
  addFrame(10);
...
TEST_F(TestGnssStereoProvider, stereoPipelineValidImuSequence) {
  addImu(0);
  addFrame(1);
  ...
  auto result_g = castUnique<GnssStereoImuSyncPacket>(std::move(output));
  ASSERT_TRUE(result_g != nullptr);
  EXPECT_EQ(GnssPoint(1, 2, 3), result_g->getGnssPoint());
}

TEST_F(TestGnssStereoProvider, stereoPipelineInvalidImuSequence) {
...
TEST_F(TestGnssStereoProvider, testPartialImuSequence) {
...
TEST_F(TestGnssStereoProvider, testOutOfOrderImuSequence) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(1);
  ...
  addImu(5);  // should be dropped
  addGnss(4);
  addGnss(3);  // should be dropped
  addGnss(5);
  addGnss(5);  // should be dropped
  addGnss(5);  // should be dropped
...
TEST_F(TestGnssStereoProvider, testOutOfOrderImageSequence) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(3);
...
TEST_F(TestGnssStereoProvider, testOutOfOrderManyImageSequence) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(3);
  addGnss(2);
...
TEST_F(TestGnssStereoProvider, testOutOfOrderImuAndImageSequence) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(3);
...
TEST_F(TestGnssStereoProvider, stereoPipelineWithCoarseCorrection) {
...
TEST_F(TestGnssStereoProvider, gnssStereoPipelineManualTimeShift) {
  test_provider_->setImuTimeShift(10.0e-9);
  test_provider_->setGnssTimeShift(10.0e-9);
...
TEST_F(TestGnssStereoProvider, dropRightFrame) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(1);
