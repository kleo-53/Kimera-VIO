...
/**
 * @file   GnssStereoDataProviderModule.h
 * @brief  Pipeline module that provides stereo data to the VIO pipeline.
 * @details Collects camera, IMU and GNSS data, publishes StereoFrames via
 *          callback getInputPacket processes one stereo pair at a time,
 *          attempting to gather IMU data between the current stereo pair and
 *          the previous stereo pair and GNSS point at current stereopair.
